...
Print logs for a service.

```bash
gdnsl logs [flags] SERVICE
```
...
```bash
  ...
  # Return snapshot logs for nginx service
  gdnsl logs nginx

  # Return snapshot of previous terminated service logs for nginx service
  gdnsl logs -p nginx

  # Begin streaming the logs for nginx service
  gdnsl logs -f nginx

  # Display only the most recent 20 lines service nginx
  gdnsl logs --tail=20 nginx

  # Show all logs from service nginx written in the last hour
  gdnsl logs --since=1h nginx
...
```
  ...
  -p, --previous[=false]         If true, print the logs for the previous instance of the container in a pod if it exists.
      --since=0                  Only return logs newer than a relative duration like 5s, 2m, or 3h. 
...
Get details of a compute command.
...
```
  ...
  -h, --help                 Help to show a compute command status.
  -r, --regions=<value>      List of region names or keywords (LOCAL or ALL). 
                             Overrides the region setting in gdnsl.yml config file. 
```
