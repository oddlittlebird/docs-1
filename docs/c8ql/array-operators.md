...
Note that the elements are not de-duplicated. For a flat array with only unique elements, a combination of [UNIQUE()](https://macrometacorp.github.io/docs-c8/reference/c8ql/functions/array/#unique) and [FLATTEN()](https://macrometacorp.github.io/docs-c8/reference/c8ql/functions/array/#flatten) is advisable.
...
The number of elements returned can be restricted with `LIMIT`. It works the same as the [limit operation](https://macrometacorp.github.io/docs-c8/reference/c8ql/operations/limit). `LIMIT` must come after `FILTER` and before `RETURN`, if they are present.
